...
export class AuthService {
    ...
    try {
      ...
      if (!user) {
        ...
        throw new HttpException(
          ...
          `This account doesn't exist or is not active. Please check your email or sign up for a new account.`,
        );
      }
      ...
      if (!isPasswordMatching) {
        // Incorrect credentials should be 401 Unauthorized with a user-friendly message
        throw new HttpException(HttpStatusCodes.UNAUTHORIZED, 'Incorrect email or password. Please try again.');
      }
  ...
  public async logout(userData: User, token: string): Promise<Omit<User, 'password'>> {
    ...
    if (!user) {
      // Updated to use a more user-friendly message
      throw new HttpException(HttpStatusCodes.NOT_FOUND, `Account not found. Please check your credentials.`);
    }
  ...
  public async requestPasswordReset(passwordResetData: RequestPasswordResetDto): Promise<{ message: string }> {
    ...
    return await DbHelper.withTransaction(async (client: PoolClient) => {
      ...
      if (!user) {
        // Updated with more user-friendly message
        throw new HttpException(
          HttpStatusCodes.NOT_FOUND,
          `We couldn't find an account with that email address. Please check your email or register for a new account.`,
        );
      }
