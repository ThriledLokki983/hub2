...
const authMiddlewareImpl = async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
...
// Create an Express-compatible middleware function
export const AuthMiddleware = (req: Request, res: Response, next: NextFunction): void => {
  authMiddlewareImpl(req as RequestWithUser, res, next).catch(error => {
    logger.error(`Unhandled error in AuthMiddleware: ${error.message}`);
    next(new HttpException(500, 'Authentication failed due to server error'));
  });
};
