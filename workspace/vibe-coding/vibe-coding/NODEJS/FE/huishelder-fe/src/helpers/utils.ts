...
/**
 * Strips double slashes from a URL string while preserving protocol slashes
 * @param str URL string to process
 * @returns URL string with double slashes removed except for protocol
 */
export const stripDoubleSlashes = (str: string): string => {
...
export const getBreakpointValues = (sizes: CssBreakPointsInterface = []) =>
  sizes.reduce((acc: Record<string, number>, size: string) => {
 ...
 */
export const getMatchingBreakpoints = (sizes: number[]): string[] => {
  return Object.entries(sizes)
    .filter(([_name, size]) => matchesBreakpoint(size))
