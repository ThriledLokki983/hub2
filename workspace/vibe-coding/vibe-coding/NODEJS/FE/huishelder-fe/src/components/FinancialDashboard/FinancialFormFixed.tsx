 ...
 * Fixed version to prevent input focus loss
 */
const FinancialForm: React.FC<FinancialFormProps> = ({ onSubmit, isLoading }) => {
  ...
  const handleChange = (field: keyof FinancialInput, value: number | boolean) => {
    setFormValues(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  // Handle focus/blur to prevent focus loss
  const handleFocus = (e: React.FocusEvent<HTMLInputElement>) => {
    setFocusedId(e.target.id);
  };
  ...
  // Custom number input component with focus management
  const NumberInput = ({
    id,
    label,
    value,
    ...
    // Save DOM reference for this input
    const inputRef = React.useRef<HTMLInputElement>(null);

    // Restore focus to this element if needed
    React.useEffect(() => {
      if (focusedId === id && inputRef.current && document.activeElement !== inputRef.current) {
