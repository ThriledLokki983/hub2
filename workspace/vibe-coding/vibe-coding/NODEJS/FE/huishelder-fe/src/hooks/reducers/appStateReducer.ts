...
const useAppStateReducer = (initState: AppState): AppStateReducer => {
  ...
  const actionCreators = useMemo(
    () => ({
      UPDATE_ROUTE: (
        ...
        title?: string,
      ): { type: 'UPDATE_ROUTE'; payload: RoutePayload } => ({
        type: 'UPDATE_ROUTE',
        payload: { route, title },
      }),
      UPDATE_PAGE_TITLE: (title: string): { type: 'UPDATE_PAGE_TITLE'; payload: TitlePayload } => ({
        type: 'UPDATE_PAGE_TITLE',
        payload: { title: title ? `${title} | ${SITE_TITLE}` : SITE_TITLE },
      }),
      UPDATE_BREAKPOINTS: (
        cssBreakPoints: number[],
      ): { type: 'UPDATE_BREAKPOINTS'; payload: BreakpointsPayload } => ({
        type: 'UPDATE_BREAKPOINTS',
        payload: { matchingBreakpoints: getMatchingBreakpoints(cssBreakPoints) },
      }),
      UPDATE_STATE_LOADING: (
        isLoading: boolean,
      ): { type: 'UPDATE_STATE_LOADING'; payload: LoadingPayload } => ({
        type: 'UPDATE_STATE_LOADING',
        payload: { isLoading },
      }),
      UPDATE_STATE_FETCHING: (
        isFetching: boolean,
      ): { type: 'UPDATE_STATE_FETCHING'; payload: FetchingPayload } => ({
  ...
  const actions: Actions = useMemo(
    () => ({
      updateRoute: (route: CustomRouteObject) => actionCreators.UPDATE_ROUTE(route),
      updatePageTitle: (title: string) => actionCreators.UPDATE_PAGE_TITLE(title),
      updateBreakPoints: (cssBreakPoints: number[]) =>
        actionCreators.UPDATE_BREAKPOINTS(cssBreakPoints),
      updateStateLoading: (isLoading: boolean) => actionCreators.UPDATE_STATE_LOADING(isLoading),
      updateStateFetching: (isFetching: boolean) =>
        actionCreators.UPDATE_STATE_FETCHING(isFetching),
    }),
    [actionCreators],
  );

  // Cast the actions to the required return type to avoid TypeScript errors
  return [state, dispatch, actions as Record<string, (...args: unknown[]) => AppStateAction>];
};
...
const appStateReducer: Record<ActionTypes, AppStateActionHandler> = {
  UPDATE_ROUTE: (state: AppState, action: AppStateAction) => {
    const payload = action.payload as RoutePayload;
    return { ...state, route: payload.route };
  },
  UPDATE_PAGE_TITLE: (state: AppState, action: AppStateAction) => {
    const payload = action.payload as TitlePayload;
    return { ...state, title: payload.title };
  },
  UPDATE_BREAKPOINTS: (state: AppState, action: AppStateAction) => {
    const payload = action.payload as BreakpointsPayload;
    return { ...state, matchingBreakpoints: payload.matchingBreakpoints };
  },
  UPDATE_STATE_LOADING: (state: AppState, action: AppStateAction) => {
    const payload = action.payload as LoadingPayload;
...
const appStateReducerFunction: Reducer<AppState, AppStateAction> = (state, action) =>
  appStateReducer[action.type](state, action);
