 ...
 * - Check the latates `digitialassistant/frontend` implementation, should contain a few
...
const useApi = <T>({ endpoint, method, json: parseJson, cache: useCache }: ApiEndpointInterface): {
  ...
  const connect = useCallback((
    ...
    const call = fetch(stripDoubleSlashes(API_URL + endpoint), {
      method: method,
      credentials: 'include',
      headers: {
      ...
      FETCH_CACHE[CACHE_KEY] = call as Promise<ApiResult<T>> ;
      return Promise.resolve(call) as Promise<ApiResult<T>>;
