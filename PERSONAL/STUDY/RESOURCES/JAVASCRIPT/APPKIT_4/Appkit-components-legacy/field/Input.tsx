...
import { useControlled, KEY_VALUES, mergeRefs } from '../utils';
...
const Input = React.forwardRef<HTMLInputElement, InputProps>((props, ref) => {
    ...
    React.useEffect(() => {
        const eventClickHandler = (event: MouseEvent) => {
            if (containerRef.current && (containerRef.current as HTMLElement).contains(event.target as HTMLElement) && clickFocus) {
                return false
            }
            else {
                setClickFocus(true);
    ...
    React.useEffect(() => {
        let tagWidth;
        if (tagRef.current) {
            tagWidth = (tagRef.current as HTMLElement).getBoundingClientRect().width;
        ...
        <div className={fieldWrapperClass} ref={ref} onKeyUp={onkeydown} {...otherProps}>
            <div className={fieldContainerClass} ref={containerRef}>
                ...
                >
                    ...
                    {phoneTag && <div className="ap-field-dropdown-phone-tag" ref={tagRef}>{selectedItem && selectedItem.descValue}</div>}
                    <input
                        type={type}
                    ...
                    {placeholder && showPlaceholder && (
                        <div aria-hidden="true" className="ap-field-placeholder-container">
                            <span className="ap-field-placeholder">{placeholder}</span>
                        </div>
                    )}
                    {
                        ...
                        >
                            ...
                            >
                                {selectedItem?.url && (
                                    <img className="ap-field-img" src={selectedItem.url} aria-hidden />
                                )}
                                {selectedItem?.unit && (
                                    <span className="ap-field-unit" aria-hidden>{selectedItem.unit}</span>
                                )}
                                <span
                    ...
                    {allowClear && (
                        <div className="ap-field-suffix-wrapper">
                            <span
                                ...
                                tabIndex={value && value.length && !readonly && !disabled ? 0 : undefined}
                                aria-label="Clear text"
                                aria-disabled={value.length === 0 || readonly || disabled}
                                onClick={handleClear}
                    ...
                    {revealer && (
                        <div className="ap-field-suffix-wrapper">
                            <span
                                ...
                                aria-label={reveal ? "Reveal password" : "Unreveal password"}
                                aria-disabled="false"
                    ...
                    {suffix && <div className="ap-field-suffix-wrapper">{suffix}</div>}
                </div>
