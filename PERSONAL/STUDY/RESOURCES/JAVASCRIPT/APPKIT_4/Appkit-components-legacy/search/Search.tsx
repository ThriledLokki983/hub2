...
export interface SearchProps {
  ...
  limitHistory?: number;
  showHistory?: boolean;
  onHistoryChange?: (histoies: ItemDataType[]) => void;
...
const Search = React.forwardRef<HTMLInputElement, SearchProps>((props, ref) => {
  ...
  const [searchValue, setSearchValue, isControlled] = useControlled(searchValueProp, defaultValue);
  ...
  items = React.useMemo(() => {
    if ('data' in props) {
      ...
      items = datalist?.filter(shouldDisplay(searchBy, searchValue, searchValue !== '' ? '' : value)) || []
      if (items.length > 0) {
        setShowBlank(false);
      } else {
        setShowBlank(true);
  ...
  const getHistories = () => {
    ...
    let histoies: ItemDataType[] = [];

    if (historiesString) {
      histoies = JSON.parse(historiesString) as ItemDataType[];
    }
    return histoies;
  }
  ...
  const handleEnter = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (searchType === 'primary') {
      if (event.key === KEY_VALUES.SPACE && !buttonEle.contains(event.target as HTMLElement)) {
        return;
      }
      handlePrimarySearch(event);
      return;
    } else if (searchType === 'secondary' && !('data' in props)) {
    ...
    const focusItem = showHistory ? getHistories().find((item: any) => item[valueKey] === focusItemValue) : items.find((item: any) => item[valueKey] === focusItemValue);
    ...
    if (value !== searchValueTemp) { // focusItemValue
  ...
  const handleSelect = useCallback(
    (value, item: ItemDataType, event: React.SyntheticEvent) => {
  ...
  const handleClose = (event?: React.SyntheticEvent, shouldClear = false) => {
    if (showList) {
      setShowList(false);
      // setFocus(false);
      onClose?.();
  ...
  const handleItemSelect = useCallback(
    (value, item: any, event: React.SyntheticEvent) => {
  ...
  const saveHistory = (item: ItemDataType) => {
    if (searchType === 'secondary' && 'data' in props && props.showHistory) {
      ...
      if (historiesString) {
        histoies = JSON.parse(historiesString) as ItemDataType[];
        if (histoies.length === limitHistory) histoies.pop();

        const index = histoies.findIndex((history: any) => history[valueKey] === item[valueKey]);
        if (index > -1) {
          return
        } else {
          histoies.unshift(item);
        }
      } else {
        histoies = [item];
      }
      localStorage.setItem(LocalStorageKey, JSON.stringify(histoies));
      onHistoryChange?.(histoies);
    }
  };
  ...
  const searchTypeNode = () => {
    ...
    const searchTypeProps = {
      ...
      tabIndex: (disabled || (searchType === 'primary' && !showPressIcon) || (searchType === 'secondary' && searchValue.length === 0)) ? -1 : 0,
      'aria-hidden': !showPressIcon,
      'aria-label':
        ariaLabel || searchType === 'primary' ? 'start search' : 'clear',
      className: classes,
      onClick: searchType === 'primary' ? handlePrimarySearch : handleClean,
    };
      ...
      if (histories.length === 0) {
        ...
        tmp = (
          <div className="ap-option-item noResult">
            <span className="ap-option-label">{noHistoryMsg}</span>
          </div>
        );
  ...
  return (
    <div className={containerClasses} ref={containerRef} {...otherProps}>
      ...
      {showList && searchType !== 'primary' && (
        <div className="ap-search-list" ref={overlayRef}>
          ...
          {(showList && showBlank && items.length === 0 && !showHistory) && (
            <div className="ap-option-item noResult">
