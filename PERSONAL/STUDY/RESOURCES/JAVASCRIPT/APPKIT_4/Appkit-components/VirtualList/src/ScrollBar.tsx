...
export default class ScrollBar extends React.Component<ScrollBarProps, ScrollBarState> {
  ...
  componentDidMount() {
    if (this.scrollbarRef.current) this.scrollbarRef.current.addEventListener('touchstart', this.onScrollbarTouchStart);
    if (this.thumbRef.current) this.thumbRef.current.addEventListener('touchstart', this.onMouseDown);
  }
