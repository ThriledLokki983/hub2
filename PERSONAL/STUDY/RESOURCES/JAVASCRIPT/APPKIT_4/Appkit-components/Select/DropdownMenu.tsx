...
const DropdownMenu = React.forwardRef<HTMLDivElement, DropdownMenuProps>(
    (props, ref) => {
        ...
        const handleSelect = useCallback(
            (value: any, item: any, e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const renderItem = (
            ...
            const isLinkItem = value && label && url;
            ...
            if (isGroup) {
                ...
                return (
                    <div
                        key={index}
                        className="ap-option-group"
                        title={getText(groupTitle)}
                    >
            ...
            const disabled =
                (!data && item.disabled) ||
                data?.disabled ||
                disabledItemValues?.includes(value);
            const active = Array.isArray(selectedValues)
                ? selectedValues.includes(value)
                : selectedValues === value;

            const focus =
                focusItemValue?.value === value && focusItemValue?.keyboard;

            let itemElement = null;

            if (itemTemplate && typeof itemTemplate === 'function') {
                itemElement = itemTemplate(label, data, url);
            }
            ...
            return (
                ...
                >
                    {itemTemplate && !isLinkItem ? itemTemplate(label, data, url) : null}
                    {/* {url && isLinkItem ? ( */}
        ...
        return (
            ...
            >
                ...
                {flattenItems.length === 0 ? (
                    ...
                    flattenItems.map((_item, index: number) =>
                        renderItem(flattenItems, { index }),
                    )
                )}
            </div>
        );
