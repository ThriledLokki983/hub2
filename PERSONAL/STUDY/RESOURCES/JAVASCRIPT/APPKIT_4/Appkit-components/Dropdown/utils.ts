...
function onMenuKeyDown(event: React.KeyboardEvent, events: EventsProps) {
    const {
        down,
        up,
        enter,
...
const useFocusItemValue = <T>(
            ...
            if (keyWord) {
                const newItems = items.filter(
                    (item) => !item.disabled
                        ...
                        && item[labelKey]?.toString()[0].toLowerCase()
                        === keyWord.toLowerCase(),
                );
                return newItems;
            }
    ...
    const findFocusItemIndex = useCallback(
        (callback: any, keyWord?: string, isUp?: boolean) => {
...
export { onMenuKeyDown, useFocusItemValue };
