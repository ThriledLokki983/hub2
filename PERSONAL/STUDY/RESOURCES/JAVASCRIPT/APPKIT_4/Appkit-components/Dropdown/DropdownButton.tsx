...
import { Button, ButtonGroup, ButtonProps } from '../AppkitButton';
import { Avatar } from '../AppkitAvatar';
...
const DropdownButton = React.forwardRef<HTMLDivElement, DropdownButtonProps>((props, ref) => {
    ...
    const displayItems = useMemo(() => flattenItemsFun(parsedItems, { childrenKey, valueKey, labelKey }), [parsedItems]);
    ...
    const buttonAriaAttributes: React.ButtonHTMLAttributes<HTMLButtonElement> = useMemo(() => ({
        ...
        'aria-controls': `${(buttonId || splitButton ? 'group' : 'menu')}dropdown`,
    ...
    const buttonProps: React.ButtonHTMLAttributes<HTMLButtonElement> = useMemo(() => ({
        id: buttonId,
    ...
    useClickOutside({
        enabled: visible,
        isOutside: (event: MouseEvent) => !popupRef.current?.contains(event.target as HTMLElement) && !targetRef.current?.contains(event.target as HTMLElement),
