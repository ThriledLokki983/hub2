...
const filterBoxHandler = () => {
	const filterBox = document.querySelector('.select__options.mob-select-opt');
	const results = document.querySelector('.select__results');

	if (filterBox) {
		const radioInputs = Array.from(
			filterBox.querySelectorAll('input[type="radio"]')
		);
		if (radioInputs.length <= 0) {
			filterBox.style.display = 'none';
			filterBox.remove();

			results?.classList.remove('span-3');
			results?.classList.add('span-4');
		} else {
			// do nothing
		}
	}
